...
class CatalogOrchestrator extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    this.catalogPath = path.join(this.projectRoot, '.bumba-design', 'design-catalog.html');
  ...
  async loadCatalogData() {
    try {
      // Check if .bumba-design directory exists
      try {
  ...
  async saveCatalogData(data) {
    try {
      // Ensure .bumba-design directory exists
      try {
        await fs.access(this.designDir);
      } catch {
        console.log('Creating .bumba-design directory...');
        await fs.mkdir(this.designDir, { recursive: true });
      }
