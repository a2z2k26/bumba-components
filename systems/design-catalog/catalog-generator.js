 ...
 * - Converts to .bumba-design/catalog-data.json (catalog format)
...
class CatalogGenerator {
  ...
  async generate({ projectDir, data, routeComponents = true }) {
    const catalogDir = path.join(projectDir, '.bumba-design');
  ...
  async routeComponentsToPages(components, options = {}) {
    ...
    // Component routing disabled - catalog is now self-contained in .bumba-design
    console.log('Component routing skipped - using self-contained catalog');
    return { routed: 0, created: 0, variants: 0, failed: 0, details: [] };
